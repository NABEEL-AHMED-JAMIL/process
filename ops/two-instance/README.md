...
T8 analytics-service, T10 billing-service. T4 (page access) and T5 (the sign-in lockout) were process's until
Identity's endpoints left it (MIG-108); process no longer grants a page or signs anyone in, so they are retired
here, and checking them against two identity-service replicas is a follow-up.

```
ops/two-instance/run.sh                                  # build process, unit gate, T1-T10
ops/two-instance/run.sh --only T1,T6,T7                  # some checks
ops/two-instance/run.sh --no-build --mutate split-broadcast  # a mechanism broken on purpose: the named checks must go red
ops/two-instance/run.sh --keep                           # leave it up; then:
python3 ops/two-instance/harness.py --attach --only T6   #   re-run checks against it
...
| T4, T5 | Retired (MIG-108): the page grant and the sign-in lockout they drove through process's `/pageAccess.json` and `/auth.json` are identity-service's. |
...
`split-login-guard` (T5) and `split-redis` (T4, T5) were retired with those checks in MIG-108.
...
- run: for m in unlocked-enqueuer unaligned-reconcile split-broadcast unaligned-close; do ops/two-instance/run.sh --no-build --no-unit --mutate "$m"; done
```
