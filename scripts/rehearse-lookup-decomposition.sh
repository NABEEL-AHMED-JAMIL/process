#!/usr/bin/env bash
# MIG-167: rehearse V140-V144 (lookup_data decomposed and retired, pipeline_config) on a full copy of a long-lived
# etl_job, then drop the copy. As scripts/rehearse-timestamptz.sh does:
#
#   scripts/rehearse-lookup-decomposition.sh [source-db]        (default etl_job)
#
# The source is only read (pg_dump). The copy is made on the same server under a new name, migrated, checked row by
# row and rolled back by LookupDecompositionRehearsal, and dropped whatever happens. Credentials come from the
# container's own environment and are never printed.
set -euo pipefail

SOURCE_DB=${1:-etl_job}
CONTAINER=${POSTGRES_CONTAINER:-postgres_db}
COPY="etl_job_lookup_rehearsal_$(date +%s)"
cd "$(dirname "$0")/.."
...
LOOKUP_REHEARSAL_DB=$COPY mvn -o -q test -Dtest='LookupDecompositionRehearsal'
