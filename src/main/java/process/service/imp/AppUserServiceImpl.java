...
public class AppUserServiceImpl implements AppUserService {
    ...
    public AppResponse getAppUserProfile(String username) throws Exception {
        ...
        if (!appUserDetail.getAppUserRoles().isEmpty()) {
            appUserResponse.setRoleResponse(appUserDetail.getAppUserRoles().stream()
                .map(role -> this.getRoleResponse(role)).collect(Collectors.toSet()));
        }
    ...
    public AppResponse updateAppUserPassword(UpdateUserProfileRequest payload) throws Exception {
        ...
        if (this.emailMessagesFactory.sendUpdateAppUserPassword(payload)) {
            this.notificationService.addNotification(this.getNotificationRequest("Dear user, your password updated."), appUser.get());
            return new AppResponse(ProcessUtil.SUCCESS, "AppUser Profile Update.", payload);
        }
        return new AppResponse(ProcessUtil.ERROR, "Account updated, Email not send contact with support.", payload);
    }
    ...
    public AppResponse getSubAppUserAccount(String username) throws Exception {
        ...
        appUserResponse.setSubAppUser(appUser.get().getAppUserChildren().stream()
            .filter(appUser1 -> appUser1.getStatus() != APPLICATION_STATUS.DELETE.getLookupValue())
    ...
    public AppResponse signInAppUser(LoginRequest payload) throws Exception {
        ...
        return new AppResponse(ProcessUtil.SUCCESS, "User successfully authenticate.",
            new AuthResponse(userDetails.getAppUserId(), jwtToken, refreshToken.getToken(), userDetails.getUsername(),
    ...
    public AppResponse signupAppUser(SignupRequest payload) throws Exception {
        ...
        Optional<AppUser> superAdmin = this.appUserRepository.findByUsernameAndStatus(
            lookupDataResponse.getLookupValue(), APPLICATION_STATUS.ACTIVE.getLookupValue());
    ...
    public ByteArrayOutputStream downloadAppUser(SignupRequest payload) throws Exception {
        ...
        appUser.get().getAppUserChildren().stream()
                ...
                .forEach(tempAppUser -> {
                    ...
                    dataCellValue.add(SCHEDULER_TIMEZONE.getSchedulerTimeZoneByValue(tempAppUser.getTimeZone()).getLookupType());
    ...
    public AppResponse uploadAppUser(FileUploadRequest payload) throws Exception {
        ...
        appUserValidations.forEach(appUserValidation -> {
            ...
            newAppUser.setParentAppUser(superAdmin.get());
            newAppUser.setTimeZone((Long) SCHEDULER_TIMEZONE.getSchedulerTimeZoneByLookupType(appUserValidation.getTimeZone()).getLookupValue());
            Optional<Role> role = Optional.empty();
            ...
            try {
                this.notificationService.addNotification(this.getNotificationRequest("Dear user, your account created successfully."), newAppUser);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            this.appUserEnvRepository.insertAppUserEnv(newAppUser.getAppUserId().intValue(), superAdmin.get().getAppUserId().intValue());
        });
    ...
    private AppUserResponse getAppUserDetail(AppUser appUser) {
        ...
        appUserResponse.setTimeZone(SCHEDULER_TIMEZONE.getSchedulerTimeZoneByValue(appUser.getTimeZone()));
