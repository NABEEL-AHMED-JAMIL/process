package process.security.oauth2;
...
public class OAuth2AccessTokenResponseConverterWithDefaults implements Converter<Map<String, String>, OAuth2AccessTokenResponse> {
    ...
    public Logger logger = LogManager.getLogger(OAuth2AccessTokenResponseConverterWithDefaults.class);

    private static final Set<String> TOKEN_RESPONSE_PARAMETER_NAMES = Stream.of(OAuth2ParameterNames.ACCESS_TOKEN,
        OAuth2ParameterNames.TOKEN_TYPE, OAuth2ParameterNames.EXPIRES_IN, OAuth2ParameterNames.REFRESH_TOKEN,
    ...
    public OAuth2AccessTokenResponse convert(Map<String, String> tokenResponseParameters) {
        ...
        if (tokenResponseParameters.containsKey(OAuth2ParameterNames.EXPIRES_IN)) {
            try {
                ...
                logger.info("Error during parse expires " + ExceptionUtil.getRootCause(ex));
            }
        }
        ...
        return OAuth2AccessTokenResponse.withToken(accessToken).tokenType(accessTokenType)
            .expiresIn(expiresIn).scopes(scopes).additionalParameters(additionalParameters).build();
    }
