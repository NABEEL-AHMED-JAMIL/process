...
public class MessageQApiServiceImpl implements MessageQApiService {
    ...
    public AppResponse fetchLogs(MessageQSearchRequest messageQSearch) {
        ...
        if (isNull(messageQSearch.getFromDate())) {
            return new AppResponse(ProcessUtil.ERROR, "FromDate missing.");
        } else if (isNull(messageQSearch.getToDate())) {
            return new AppResponse(ProcessUtil.ERROR, "ToDate missing.");
        }
        ...
        if (!isNull(result) && result.size() > 0) {
            ...
            responseDto = new AppResponse(ProcessUtil.SUCCESS, "MessageQ successfully ", objectMap);
        } else {
            responseDto = new AppResponse(ProcessUtil.SUCCESS, "No Data found for sourceTask.", new ArrayList<>());
        }
        return responseDto;
    }
    ...
    public AppResponse failJobLogs(Long jobQId) {
        if (isNull(jobQId)) {
            return new AppResponse(ERROR, "JobQId missing.");
        }
        ...
        return new AppResponse(ERROR, "JobQueue not found");
    }
    ...
    public AppResponse changeJobStatus(QueueMessageStatusRequest queueMessageStatus) {
        if (isNull(queueMessageStatus.getMessageType())) {
            return new AppResponse(ERROR, "Message Type required for transaction.");
        }
        ...
        return new AppResponse(SUCCESS, "QueueMessage Successfully Update.");
    }
