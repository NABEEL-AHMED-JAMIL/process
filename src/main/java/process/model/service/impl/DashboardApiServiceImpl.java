...
public class DashboardApiServiceImpl implements DashboardApiService {
    ...
    public AppResponse jobStatusStatistics() throws Exception {
        ...
        return new AppResponse(SUCCESS, "No Data found for jobStatusStatistics.", new ArrayList<>());
    }
    ...
    public AppResponse jobRunningStatistics() throws Exception {
        ...
        return new AppResponse(SUCCESS, "No Data found for jobRunningStatistics.", new ArrayList<>());
    }
    ...
    public AppResponse weeklyRunningJobStatistics(String startDate, String endDate) throws Exception {
        ...
        return new AppResponse(SUCCESS, "No Data found for weeklyRunningJobStatistics.", new ArrayList<>());
    }
    ...
    public AppResponse weeklyHrsRunningJobStatistics(String startDate, String endDate) throws Exception {
        ...
        if (!isNull(result) && result.size() > 0) {
            List<WeeklyJobStatisticsResponse> weeklyJobStatistics = new ArrayList<>();
            for(Object[] obj : result) {
                int index = 0;
                weeklyJobStatistics.add(new WeeklyJobStatisticsResponse(obj[index].toString().trim(),
                    Double.valueOf(obj[++index].toString()).longValue(), obj[++index].toString().trim(),
                    Double.valueOf(obj[++index].toString()).longValue()));
            }
            responseDto = new AppResponse(SUCCESS, "Data found for weeklyHrsRunningJobStatistics.", weeklyJobStatistics);
        } else {
            responseDto = new AppResponse(SUCCESS, "No Data found for weeklyHrsRunningJobStatistics.", new ArrayList<>());
        }
        return responseDto;
    }
    ...
    public AppResponse weeklyHrRunningStatisticsDimension(String targetDate, Long targetHr) throws Exception {
        ...
        if (!isNull(result) && result.size() > 0) {
            ...
            responseDto = new AppResponse(SUCCESS, "Data found for weeklyHrRunningStatisticsDimensionData.", weeklyJobStatistics);
        } else {
            responseDto = new AppResponse(SUCCESS, "No Data found for weeklyHrRunningStatisticsDimensionData.", new ArrayList<>());
        }
        return responseDto;
    }

    @Override
    public AppResponse weeklyHrRunningStatisticsDimensionDetail(String targetDate, Long targetHr,
        ...
        if (!isNull(result) && result.size() > 0) {
            ...
            responseDto = new AppResponse(SUCCESS, "Data found for weeklyHrRunningStatisticsDimensionDetail.", objectDetail);
        } else {
            responseDto = new AppResponse(SUCCESS, "No Data found for weeklyHrRunningStatisticsDimensionDetail.");
        }
        return responseDto;
    }
