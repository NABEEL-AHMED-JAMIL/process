 ...
 * passed in (Chicago wall-clock, bound as the instant it means -- BusinessTime.timestampOf), as they
 * always have been; received_at is timestamptz since V100.
 *
 * @author Nabeel Ahmed
 */
@Component
public class JdbcCallbackReceipts implements CallbackReceipts {
    ...
    public Optional<Receipt> claim(Long jobQueueId, String key, String request, LocalDateTime receivedAt) {
        int claimed = this.jdbc.update("INSERT INTO worker_callback_receipt (job_queue_id, idempotency_key, request, received_at) "
            ...
            jobQueueId, key, request, BusinessTime.timestampOf(receivedAt));
    ...
    public int purgeReceivedBefore(LocalDateTime cutoff) {
        return this.jdbc.update("DELETE FROM worker_callback_receipt WHERE received_at < ?", BusinessTime.timestampOf(cutoff));
    }
}
