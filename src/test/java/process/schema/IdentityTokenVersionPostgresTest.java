...
/**
 * MIG-14 against a real Postgres: app_user.token_version (V66) and the query that reads it.
 * TokenRevocationAcrossInstancesTest runs the same read against a map; this proves the SQL, and that
 * saving an entity loaded before a bump cannot put the old version back. The bump itself is
 * identity-service's since MIG-108, so here it is the statement it runs. Opt-in (IdentityPostgres).
 */
class IdentityTokenVersionPostgresTest {

    @Test
    void theVersionStartsAtZeroIsReadBackAndSurvivesAStaleSave() throws Exception {
        try (IdentityPostgres db = IdentityPostgres.create("mig14_version").migrate().withJpa()) {
            ...
            sql.update("UPDATE app_user SET token_version = token_version + 1 WHERE app_user_id = 9001");
            ...
            assertThat(users.findTokenVersion(9002L)).isZero();
